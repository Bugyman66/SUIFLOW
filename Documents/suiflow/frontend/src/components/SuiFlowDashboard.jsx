...
const SuiFlowDashboard = () => {
  ...
  async function addProduct(e) {
    ...
    try {
      ...
      setNewProduct({ name: '', priceInSui: '', description: '', redirectURL: '', webhookURL: '' });
      await fetchProducts();
    } catch (e) {
    ...
    <div className="sui-dashboard">
      ...
      <div className="sui-sidebar">
        ...
        <nav className="sui-nav">
          <button 
            className={`sui-nav-item ${activeTab === 'dashboard' ? 'active' : ''}`}
          ...
          <button 
            className={`sui-nav-item ${activeTab === 'payments' ? 'active' : ''}`}
          ...
          <button 
            className={`sui-nav-item ${activeTab === 'transactions' ? 'active' : ''}`}
          ...
          <button 
            className={`sui-nav-item ${activeTab === 'webhooks' ? 'active' : ''}`}
          ...
          <button 
            className={`sui-nav-item ${activeTab === 'settings' ? 'active' : ''}`}
        ...
        <div className="sui-header">
          <div className="sui-header-content">
            ...
            <div className="sui-merchant-info">
              <span className="sui-merchant-name">{merchant.businessName}</span>
          ...
          {activeTab === 'payments' && (
            <div className="sui-payments-section">
              <div className="sui-create-payment">
                ...
                <form onSubmit={addProduct} className="sui-payment-form">
                  ...
                  <div className="sui-form-group">
                    <input
                      ...
                      onChange={e => setNewProduct(p => ({ ...p, redirectURL: e.target.value }))}
                      type="url"
                  ...
                  <div className="sui-form-group">
                    <input
                      ...
                      onChange={e => setNewProduct(p => ({ ...p, webhookURL: e.target.value }))}
                      type="url"
                ...
                {productError && <div className="sui-error">{productError}</div>}
              </div>
                  ...
                  <div key={product._id} className="sui-product-card">
                    <div className="sui-product-info">
                      ...
                      <div className="sui-product-meta">
                        <span className="sui-price">{parseFloat(product.priceInSui).toFixed(4)} SUI</span>
                        <span className="sui-merchant-address">{product.merchantAddress}</span>
                      </div>
                    </div>
                    ...
                    <div key={payment._id} className="sui-transaction-card">
                      <div className="sui-transaction-header">
                        <div className="sui-transaction-info">
                          ...
                          <span className="sui-transaction-id">#{payment._id.slice(-8)}</span>
                        </div>
                        <div className={`sui-transaction-status ${payment.status}`}>
                          {payment.status === 'completed' || payment.status === 'paid' ? '✅ Completed' : 
                      ...
                      <div className="sui-transaction-details">
                        <div className="sui-transaction-row">
                          ...
                          <span className="sui-transaction-value">
                            {parseFloat(payment.amountInSui || payment.amount).toFixed(4)} SUI
                          </span>
                        </div>
                        
                        {payment.txnHash && (
                          <div className="sui-transaction-row">
                            <span className="sui-transaction-label">Transaction Hash:</span>
                            <span className="sui-transaction-value sui-hash">
                              {payment.txnHash.slice(0, 8)}...{payment.txnHash.slice(-8)}
                            </span>
                          </div>
                        )}
                        ...
                        {payment.customerWallet && (
                          <div className="sui-transaction-row">
                            <span className="sui-transaction-label">Customer Wallet:</span>
                            <span className="sui-transaction-value sui-hash">
                              {payment.customerWallet.slice(0, 8)}...{payment.customerWallet.slice(-8)}
                            </span>
                          </div>
                        )}
                        ...
                        {payment.paidAt && (
                          <div className="sui-transaction-row">
                            <span className="sui-transaction-label">Paid At:</span>
                            <span className="sui-transaction-value">
                      ...
                      {payment.description && (
                        <div className="sui-transaction-description">
                          <span className="sui-transaction-label">Description:</span>
            ...
            <div className="sui-webhooks-section">
              ...
              <div className="sui-webhook-config">
                <div className="sui-form-group">
                  ...
                  <input type="url" placeholder="https://your-domain.com/webhook" className="sui-input" />
                </div>
                <div className="sui-form-group">
                  <label>Secret Key</label>
                  <input type="text" placeholder="Your webhook secret" className="sui-input" />
                </div>
                <button className="sui-button sui-button-primary">Save Webhook</button>
              </div>
            </div>
          )}
              ...
              <div className="sui-settings-grid">
                <div className="sui-setting-card">
                  ...
                  <div className="sui-form-group">
                    ...
                    <input type="text" value={merchant.businessName} className="sui-input" />
                  </div>
                  <div className="sui-form-group">
                    <label>Email</label>
                    <input type="email" value={merchant.email} className="sui-input" />
                  </div>
                </div>
                ...
                <div className="sui-setting-card">
                  ...
                  <div className="sui-wallet-display">
                    <span className="sui-wallet-address">{merchant.walletAddress}</span>
                    <button className="sui-button sui-button-secondary">Copy</button>
                  </div>
                </div>
              </div>
            </div>
